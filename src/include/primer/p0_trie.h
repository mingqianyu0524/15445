...
class TrieNode {
 public:
  ...
  std::unique_ptr<TrieNode> *GetChildNode(char key_char) {
    return children_.find(key_char) == children_.end() ? nullptr : &(children_.at(key_char));
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    for (char i : key) {
      auto child_node = cur_node->get()->GetChildNode(i);
      if (child_node == nullptr) {
        cur_node->get()->InsertChildNode(i, std::make_unique<TrieNode>(i));
      }
      parent = cur_node;
      cur_node = cur_node->get()->GetChildNode(i);
    }
    if (cur_node->get()->IsEndNode()) {
      latch_.WUnlock();
      return false;
    }
    auto end_node = std::make_unique<TrieNodeWithValue<T>>(std::move(**cur_node), value);
    (*parent)->RemoveChildNode(key[key.size() - 1]);
    (*parent)->InsertChildNode(key[key.size() - 1], std::move(end_node));
    latch_.WUnlock();
    return true;
  }
  ...
  bool RemoveHelper(std::unique_ptr<TrieNode> *curr, size_t i, const std::string &key, bool *success) {
    ...
    auto child_node = (*curr)->GetChildNode(key[i]);
    if (child_node == nullptr) {
      *success = false;
      return false;
    }
    bool can_remove = RemoveHelper(child_node, i + 1, key, success);
    if (!*success) {
      return false;
    }
    if (can_remove) {
